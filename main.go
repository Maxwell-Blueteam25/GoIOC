...
func InvokeSweep(inputJsonPath string, reportMode, smbPath, sasUrl string) {
	...
	var masterReport []engines.ScanResult
	...
	for _, item := range config.Sweeps {
		...
		switch item.Type {
		case "file_hash":
			findings = engines.HashModule(item.Value, item.Constraints.Path)
		case "registry_value":
			findings = engines.RegModule(item.Value, "", item.Constraints.Path)
		case "registry_key":
			findings = engines.RegModule("", item.Value, "")
		case "process_name":
			findings = engines.ProcessModule(item.Value)
		case "file_name":
			findings = engines.FileModule(item.Value, item.Constraints.Path)
		}

		if len(findings) > 0 {
			masterReport = append(masterReport, findings...)
		}
	}
	...
	reportData, err := json.MarshalIndent(masterReport, "", "  ")
	if err != nil {
...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage: sweeper.exe <config.json> [Local|Cloud|SMB] [SMBPath] [SasUrl]")
		// Defaulting for dev testing
		InvokeSweep("sweeper_config.json", "Local", "", "")
		return
	}
	...
	InvokeSweep(jsonPath, mode, smb, sas)
}
